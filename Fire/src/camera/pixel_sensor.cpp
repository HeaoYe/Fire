...
namespace Fire {
    static constexpr Int n_swatch_reflectances = 24;
    static const PiecewiseLinearSD swatch_reflectances[n_swatch_reflectances] = {
        PiecewiseLinearSD::FromInterleaved(
            std::vector {
    ...
    static Point3r InnerProduct(const SpectrumDistribution &reflectance, const SpectrumDistribution &illuminant, const SpectrumDistribution &r_mf, const SpectrumDistribution &g_mf, const SpectrumDistribution &b_mf) {
        Point3r result {};
        Real integral {};
        for (Int lambda = g_lambda_min; lambda <= g_lambda_max; lambda ++) {
            integral += g_mf.sample(lambda) * illuminant.sample(lambda);
            result.r += reflectance.sample(lambda) * r_mf.sample(lambda) * illuminant.sample(lambda);
            result.g += reflectance.sample(lambda) * g_mf.sample(lambda) * illuminant.sample(lambda);
            result.b += reflectance.sample(lambda) * b_mf.sample(lambda) * illuminant.sample(lambda);
        }
        return result / integral;
    }

    PixelSensor::PixelSensor(const SpectrumDistribution &r_mf, const SpectrumDistribution &g_mf, const SpectrumDistribution &b_mf, const ColorSpace &output_colorspace, const SpectrumDistribution &sensor_illuminant, Real imaging_ratio)
        : r_mf(r_mf), g_mf(g_mf), b_mf(b_mf), imaging_ratio(imaging_ratio) {
        Matrix<Real, n_swatch_reflectances, 3> rgbs, xyzs;
        Real sensor_white_g_integral = SpectrumDistributionInnerProduct(sensor_illuminant, g_mf);
        Real sensor_white_y_integral = SpectrumDistributionInnerProduct(sensor_illuminant, g_cie_y_matching_function);
        for (Int i = 0; i < n_swatch_reflectances; i ++) {
            auto rgb = RGB { InnerProduct(swatch_reflectances[i], sensor_illuminant, r_mf, g_mf, b_mf) };
            rgbs.set(i, 0, rgb.r);
            rgbs.set(i, 1, rgb.g);
            rgbs.set(i, 2, rgb.b);

            auto xyz = XYZ { InnerProduct(swatch_reflectances[i], output_colorspace.getIllumiant(), g_cie_x_matching_function, g_cie_y_matching_function, g_cie_z_matching_function) } * sensor_white_y_integral / sensor_white_g_integral;
            xyzs.set(i, 0, xyz.x);
            xyzs.set(i, 1, xyz.y);
            xyzs.set(i, 2, xyz.z);
        }
        xyz_from_sensor_rgb = LinearLeastSquares(rgbs, xyzs);
    }

    PixelSensor::PixelSensor(const ColorSpace &output_colorspace, const SpectrumDistribution &sensor_illuminant, Real imaging_ratio)
        : r_mf(g_cie_x_matching_function), g_mf(g_cie_y_matching_function), b_mf(g_cie_z_matching_function), imaging_ratio(imaging_ratio) {
        if ((&sensor_illuminant) != (&output_colorspace.getIllumiant())) {
            xyY sensor_white = xyY::FromXYZ(XYZ::FromSpectrumDistribution(sensor_illuminant));
            xyz_from_sensor_rgb = xyY::WhiteBalance(sensor_white, output_colorspace.getW());
        } else {
            xyz_from_sensor_rgb = Matrix3r::GenerateIdentity();
        }
    }

    FIRE_API RGB PixelSensor::measureRadiance(const SpectrumSample &radiance_sample) const {
        auto radiance_div_pdf = SafeDiv(radiance_sample.value, radiance_sample.wavelengths_sample.getWavelengthsPDF());
        return imaging_ratio * RGB {
            (SpectrumSampleValue::FromSpectrumDistribution(r_mf, radiance_sample.wavelengths_sample) * radiance_div_pdf).average(),
            (SpectrumSampleValue::FromSpectrumDistribution(g_mf, radiance_sample.wavelengths_sample) * radiance_div_pdf).average(),
            (SpectrumSampleValue::FromSpectrumDistribution(b_mf, radiance_sample.wavelengths_sample) * radiance_div_pdf).average()
        };
    }
}
